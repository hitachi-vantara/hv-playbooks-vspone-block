...
An Ansible role for migrating GAD (Global-Active Device) pairs on Hitachi VSP One Block storage systems. This role performs post-GAD activities including non-destructive migration via swap-split operations followed by GAD pair deletion from the source system.
...
2. **Performing swap-split migration** - Non-destructively migrates GAD pairs to make secondary volumes primary
...
- **Non-destructive migration**: Uses swap-split operations to preserve data integrity
...
- Performs non-destructive swap-split operations
